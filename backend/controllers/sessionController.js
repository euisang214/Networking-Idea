...
const SessionController = {
  ...
  createSession: async (req, res, next) => {
    try {
      ...
      if (!professionalId || !startTime || !endTime) {
        throw new ValidationError('Professional ID, start time, and end time are required');
      }
  ...
  getSession: async (req, res, next) => {
    try {
      ...
      const isAuthorized = 
        ...
        (session.professional.user && session.professional.user._id.toString() === userId);
  ...
  getProfessionalSessions: async (req, res, next) => {
    try {
      ...
      // Get sessions
      const sessions = await SessionService.getProfessionalSessions(professionalProfile._id);
  ...
  processPayment: async (req, res, next) => {
    try {
      ...
      // Process payment
      const paymentResult = await PaymentService.processSessionPayment(sessionId, paymentMethodId, userId);
  ...
  addFeedback: async (req, res, next) => {
    try {
      ...
      // Add feedback
      const session = await SessionService.addFeedback(sessionId, userId, rating, comment);
  ...
  checkAvailability: async (req, res, next) => {
    try {
      ...
      // Check availability
      const isAvailable = await SessionService.checkAvailability(professionalId, startTime, endTime);
