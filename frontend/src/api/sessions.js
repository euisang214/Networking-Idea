...
const SessionsAPI = {
  ...
  getUserSessions: async (status, page = 1, limit = 10) => {
    ...
    const response = await api.get(`/sessions/user/me?${queryParams.toString()}`);
  ...
  getProfessionalSessions: async (status, page = 1, limit = 10) => {
    ...
    const response = await api.get(`/sessions/professional/me?${queryParams.toString()}`);
    return response.data;
  },
  ...
  processPayment: async (sessionId, paymentMethodId) => {
    const response = await api.post(`/sessions/${sessionId}/payment`, { paymentMethodId });
    return response.data.data.payment;
  },
  
  // Add feedback to session
  addFeedback: async (sessionId, rating, comment) => {
    const response = await api.post(`/sessions/${sessionId}/feedback`, { rating, comment });
    return response.data.data.session;
  },
