    ...
    <div className="border border-gray-200 rounded-lg overflow-hidden hover:shadow-md transition-shadow duration-200">
      <div className="p-4">
        <div className="flex justify-between items-start">
          <div>
            <div className="flex items-center">
              <span
                className={`px-2.5 py-0.5 rounded-full text-xs font-medium ${getStatusColor(session.status)}`}
              >
                {session.status}
              </span>
              <span className="mx-2 text-gray-300">•</span>
              <span
                className={`px-2.5 py-0.5 rounded-full text-xs font-medium ${getPaymentStatusColor(session.paymentStatus)}`}
              >
        ...
        <div className="mt-3">
          <ProfileStrip
            user={
              userType === "user" ? session.professional.user : session.user
            }
            role={userType === "user" ? "professional" : "candidate"}
          />
        </div>
